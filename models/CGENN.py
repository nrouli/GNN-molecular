...
class CGGNN(nn.Module):
    ...
    def forward(self, data):
        ...
        # Center positions per graph (translation invariance)
        pos = pos - scatter(pos, batch, dim=0, reduce='mean')[batch]

        # Node features: scalars (chemistry) + vector (position)
        scalars = self.algebra.embed(data.x.unsqueeze(-1), (0,))
        vectors = self.algebra.embed(pos.unsqueeze(1), (1, 2, 3))
        h = torch.cat([scalars, vectors], dim=1)

        # Edge features: bond types + squared distance
        dist_sq = (pos[row] - pos[col]).pow(2).sum(-1, keepdim=True)
        edge_mv = self.algebra.embed(dist_sq.unsqueeze(-1), (0,))

        # Message passing + readout
        h = self._forward(h, (row, col), edge_mv)
        return global_add_pool(h[..., 0], batch)
