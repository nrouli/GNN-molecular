...
class CliffordAlgebra(nn.Module):
    def __init__(self, metric):
        ...
        self.register_buffer("bbo_grades", bbo_grades)
        self.register_buffer("even_grades", bbo_grades % 2 == 0)
        self.register_buffer("odd_grades", ~(bbo_grades % 2 == 0))
        self.register_buffer("cayley", cayley)

        # ── Precompute involution signs as buffers (not cached_property) ──
        self.register_buffer(
            "_alpha_signs", torch.pow(-1, bbo_grades)
        )
        self.register_buffer(
            "_beta_signs", torch.pow(-1, bbo_grades * (bbo_grades - 1) // 2)
        )
        self.register_buffer(
            "_gamma_signs", torch.pow(-1, bbo_grades * (bbo_grades + 1) // 2)
        )

        # ── Precompute quadratic form signs for fast norm computation ──
        # q(x) = scalar part of beta(x)*x = sum_i beta_sign_i * x_i^2 * cayley[i,0,i]
        # This exploits that cayley[i,0,k] = 0 for i != k (orthogonal basis blades)
        diag = torch.arange(self.n_blades)
        self.register_buffer(
            "_q_signs", self._beta_signs * cayley[diag, 0, diag]
        )
    ...
    def q(self, mv, blades=None):
        """Fast quadratic form using precomputed diagonal signs."""
        if blades is not None:
            return (self._q_signs[blades] * mv ** 2).sum(-1, keepdim=True)
        return (self._q_signs * mv ** 2).sum(-1, keepdim=True)
    ...
    def norms(self, mv, grades=None):
        """Fast per-grade norms; no geometric products needed."""
        if grades is None:
            grades = self.grades
        q_vals = self._q_signs * mv ** 2
        return [
            self._smooth_abs_sqrt(
                q_vals[..., self.grade_to_slice[g]].sum(-1, keepdim=True)
            )
            for g in grades
        ]

    def qs(self, mv, grades=None):
        """Fast per-grade quadratic forms; no geometric products needed."""
        if grades is None:
            grades = self.grades
        q_vals = self._q_signs * mv ** 2
        return [
